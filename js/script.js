...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('scroll', () => {
    ...
    const scale = Math.min((scrollY - threshold) / (maxScroll - threshold), 1);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const closeBtn = navMobile?.querySelector(".nav-mobile__close");

  toggleBtn?.addEventListener("click", () => {
    navMobile.classList.toggle("open");
    toggleBtn.textContent = navMobile.classList.contains("open") ? "Close" : "Menu";
  });
